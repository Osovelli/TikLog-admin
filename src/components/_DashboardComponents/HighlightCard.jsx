...
export const HighlightCard = ({ icon, currentValue, growth, label }) => {
  const hasIncreased = growth > 0
  const hasDecreased = growth < 0
  const isUnchanged = growth === 0
  ...
  const Icon = () => {
    switch (icon) {
      ...
      case "store":
        return <Store className="w-5 h-5 text-gray-500" />
      case "bicycle":
        return <FaBicycle className="w-5 h-5 text-gray-500" />
      case "users":
        return <Users className="w-5 h-5 text-gray-500" />
      default:
        return null
  ...
  const renderGrowthIndicator = () => {
    ...
    return (
      <div className={`text-sm flex items-center gap-1 ${hasIncreased ? "text-green-600" : "text-red-600"}`}>
        {hasIncreased ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
        <span>
          {Math.abs(growth).toFixed(1)}% {hasIncreased ? "growth" : "decline"}
        </span>
      </div>
    )
  }
