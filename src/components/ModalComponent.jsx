    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[100]">
      <div className='bg-white rounded-lg w-full max-w-md flex flex-col max-h-[calc(100vh-2rem)]'>
        ...
        <div className="px-6 pb-6 text-center flex-1">
          ...
          {title && <h2 className="text-xl font-semibold mb-2">{title}</h2>}
          <div className="text-gray-600 mb-6 overflow-y-auto max-h-[60vh]">
            {children}
          </div>
          <div className="flex flex-col md:flex-row w-full px-10 gap-2 mt-auto">
            {buttons?.map((button, index) => {
               // Base styles that will always be applied
               const baseStyles = "py-3 px-4 rounded-lg w-full flex-shrink";
              ...
              <button
                ...
                className={`${finalClassName} ${button.disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {button.label}
              </button>
            )})}
