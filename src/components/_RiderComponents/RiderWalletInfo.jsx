...
export const RiderWalletInfo = ({wallet}) => {
  ...
  const getTransactionTypeLabel = (type, status) => {
    switch (type.toLowerCase()) {
      ...
      case "transfer":
        return status === "completed" ? "Transfer Received" : "Transfer Sent"
      case "tip":
        return "Tip Payment"
      default:
  ...
  // Transform API data to table format
    const transactionsData = useMemo(() => {
      ...
      return transactions?.map((transaction, index) => {
        ...
        const isCredit =
          transaction.status === "Credited" || (transaction.status === "Successful" && !transaction.type)
        ...
        return {
          ...
          date: new Date(transaction.created_at || transaction.createdDate).toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
            year: "2-digit"
          }),
