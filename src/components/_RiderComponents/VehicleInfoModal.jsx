...
const ColorPicker = ({ color, onChange }) => {
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded-sm border" style={{ backgroundColor: color.toLowerCase() }} />
          <span className="text-gray-900">{color}</span>
        </div>
        <ChevronDown className={`w-5 h-5 text-gray-500 transition-transform ${isOpen ? "transform rotate-180" : ""}`} />
      </button>
          ...
          <div className="grid grid-cols-3 gap-2">
            {colors.map((colorOption) => (
              <button
                ...
                className="flex items-center gap-2 p-2 rounded hover:bg-gray-50"
              >
                <div className="w-5 h-5 rounded-sm border" style={{ backgroundColor: colorOption.toLowerCase() }} />
                <span className="text-sm">{colorOption}</span>
              </button>
            ))}
          </div>
  ...
  return (
    <div className="relative">
      ...
      <label htmlFor={`image-upload-${index}`} className="cursor-pointer block">
        {preview ? (
          <div className="aspect-square rounded-lg overflow-hidden">
            <img src={preview || "/placeholder.svg"} alt="Vehicle" className="w-full h-full object-cover" />
          </div>
        ) : (
          <div className="aspect-square bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200 transition-colors">
            <svg
...
export const VehicleInfoModal = ({ isOpen, onClose, vehicle }) => {
  ...
  // Driver options - you can expand this list as needed
  const driverOptions = ["Remi Aluko", "John Smith", "Sarah Johnson", "Michael Brown", "Emily Davis"]
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Vehicle Information">
      <div className="space-y-4">
        {/* Vehicle Make */}
        <CustomDropdown
          value={vehicleData.make}
        ...
        {/* Color Picker */}
        <ColorPicker color={vehicleData.color} onChange={(color) => setVehicleData({ ...vehicleData, color })} />

        {/* License Plate - Made it editable with common plate formats */}
        <CustomDropdown
          value={vehicleData.licensePlate}
          options={[vehicleData.licensePlate, "ABC - 1234 - DEF", "XYZ - 5678 - GHI", "LMN - 9012 - OPQ"]}
        ...
        <div className="grid grid-cols-5 gap-4">
          {[...Array(5)].map((_, index) => (
            <ImageUpload key={index} index={index} onImageChange={handleImageChange} />
          ))}
        </div>

        {/* Driver Name - Now with actual options */}
        <CustomDropdown
          value={vehicleData.driverName}
