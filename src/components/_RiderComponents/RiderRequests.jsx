...
export const RiderRequests = ({deliveries}) => {
  const columns = [
...
export const RiderRequests = ({ deliveries = [] }) => {
  ...
  const handleViewClick = (row) => {
    console.log("View delivery request:", row.rawDelivery)
    // You can implement navigation to detailed view here
    // For example: navigate(`/deliveries/${row.id}`)
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Showing {filteredData.length} of {transformedData.length} requests
          </p>
        </div>
