...
export const LicenseForm = ({ formData, onInputChange, onSave, loading, riderId }) => {
  const { getRiderVehicleById, loading: vehicleLoading, riderVehicle } = useUserStore()
  const [selectedVehicleIndex, setSelectedVehicleIndex] = useState(0)

  useEffect(() => {
    const id = riderId || formData?.riderId
    if (id) {
      getRiderVehicleById(id)
    }
  }, [getRiderVehicleById, riderId, formData?.riderId])

  // Get the vehicles array from the API response
  const vehicles = useMemo(() => {
    if (!riderVehicle) return []
    // Handle both response.data (array) and direct array
    return Array.isArray(riderVehicle) ? riderVehicle : riderVehicle?.data || []
  }, [riderVehicle])
